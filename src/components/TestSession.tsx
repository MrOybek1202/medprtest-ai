...
export default function TestSession({ userId, topic, onComplete }: TestSessionProps) {
  const [questions, setQuestions] = useState<Question[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [session, setSession] = useState<Session | null>(null);
  const [selectedOption, setSelectedOption] = useState<string | null>(null);
  const [isSubmitted, setIsSubmitted] = useState(false);
  const [mobileView, setMobileView] = useState<'question' | 'explanation'>('question');
  ...
  useEffect(() => {
    const initSession = async () => {
      ...
      try {
        ...
        if (activeSession && activeSession.question_ids && activeSession.question_ids.length > 0) {
          ...
          if (qs && qs.length > 0) {
            ...
            if (validQs.length === 0) {
              ...
              const sess = await sessionService.createSession(userId, validQs.length, topic, qIds);
              
              // If we are resuming topic progress, update the session's current index
              if (topic && startIndex > 0) {
                await sessionService.updateSession(sess.id, { current_question_index: startIndex });
              }
        ...
        toast.error('Testni boshlashda xatolik yuz berdi: ' + (error.message || 'Noma\'lum xatolik'));
      } finally {
        setLoading(false);
  ...
  const currentQuestion = questions[currentIndex];
  const cleanedStem = useMemo(() => cleanText(currentQuestion?.stem), [currentQuestion]);
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-[#0F172A] gap-4">
      <motion.div 
        ...
        className="w-12 h-12 border-4 border-[#1B4D3E] border-t-transparent rounded-full"
      />
  ...
  if (questions.length === 0) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-[#0F172A] gap-6 p-8 text-center">
      ...
      <div className="space-y-2">
        <h3 className="text-xl font-bold text-slate-900 dark:text-white">Savollar topilmadi</h3>
        <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto">Hozircha bazada savollar mavjud emas. Iltimos, keyinroq qayta urinib ko'ring.</p>
      </div>
      <button 
        onClick={onComplete}
        className="bg-[#1B4D3E] text-white font-bold px-8 py-3 rounded-2xl hover:bg-[#153a2f] transition-all"
      >
    ...
    <div className="h-full bg-[#0F172A] flex flex-col overflow-hidden text-slate-200 safe-area-top">
      ...
      <header className="bg-[#1E293B] border-b border-slate-700 px-4 md:px-6 py-3 flex justify-between items-center shrink-0">
        <div className="flex items-center gap-2 md:gap-4">
          <button onClick={onComplete} className="hover:bg-slate-700 p-2 rounded-lg transition-colors">
            <ChevronLeft size={20} />
          </button>
          <div className="h-6 w-px bg-slate-700 hidden sm:block" />
          <div className="flex items-center gap-2 md:gap-3">
            <span className="text-[10px] md:text-sm font-bold text-slate-400 whitespace-nowrap">{currentIndex + 1} / {questions.length}</span>
        ...
        <div className="flex items-center gap-3 md:gap-6">
          <div className="flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1 bg-slate-800 rounded-lg border border-slate-700">
            ...
            <span className="font-mono font-bold text-[10px] md:text-sm">{Math.floor(timeSpent / 60).toString().padStart(2, '0')}:{(timeSpent % 60).toString().padStart(2, '0')}</span>
          </div>
          ...
          <div className="flex items-center gap-1">
            <button className="p-1.5 hover:bg-slate-700 rounded-lg text-slate-400"><Settings size={16} /></button>
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        {/* Left Panel: Question */}
        <div className={`w-full lg:w-[45%] overflow-y-auto scrolling-touch bg-white dark:bg-slate-900 text-slate-900 dark:text-white p-4 md:p-8 border-r border-slate-200 dark:border-slate-800 ${isSubmitted && mobileView === 'explanation' && 'hidden lg:block'}`}>
          <div className="max-w-3xl mx-auto space-y-6 md:space-y-8 min-h-full flex flex-col justify-center py-4 md:py-8">
            <div className="space-y-4">
              <div className="flex items-center gap-2 text-[#1B4D3E] font-bold text-[10px] uppercase tracking-[0.2em]">
                <div className="w-1.5 h-1.5 rounded-full bg-[#1B4D3E]" />
                Klinik holat
              </div>
              <h3 id="question-stem-container" className="text-xl font-medium leading-relaxed select-text text-slate-900 dark:text-white">
                {cleanedStem}
              </h3>
            </div>
            ...
            <div className="space-y-4">
              {['A', 'B', 'C', 'D'].map((key) => {
                const optionText = cleanText(currentQuestion[`option_${key.toLowerCase()}` as keyof Question] as string);
                const isSelected = selectedOption === key;
                const isCorrect = currentQuestion.correct_option === key;
                
                let style = 'border-slate-100 dark:border-slate-800 hover:border-slate-200 dark:hover:border-slate-700 hover:bg-slate-50/50 dark:hover:bg-slate-800/50';
                let iconStyle = 'border-slate-200 dark:border-slate-700 text-transparent';
                
                if (isSelected) {
                  style = 'border-[#1B4D3E]/20 bg-[#1B4D3E]/5 dark:bg-[#1B4D3E]/10';
                  iconStyle = 'border-[#1B4D3E] bg-[#1B4D3E] text-white';
                }
                
                if (isSubmitted) {
                  if (isCorrect) {
                    style = 'border-green-100 dark:border-green-900/30 bg-green-50/50 dark:bg-green-900/20';
                    iconStyle = 'border-green-500 bg-green-500 text-white';
                  } else if (isSelected) {
                    style = 'border-red-100 dark:border-red-900/30 bg-red-50/50 dark:bg-red-900/20';
                ...
                return (
                  ...
                  >
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${iconStyle}`}>
                      {isSubmitted && isCorrect ? <CheckCircle2 size={14} /> : isSubmitted && isSelected ? <XCircle size={14} /> : <div className="w-2 h-2 rounded-full bg-white opacity-0 group-hover:opacity-20" />}
                    </div>
                    <span className="flex-1 text-slate-600 dark:text-slate-300 font-medium text-base leading-snug group-hover:text-slate-900 dark:group-hover:text-white transition-colors">{optionText}</span>
                  </button>
                );
              })}
            </div>
              ...
              <div className="space-y-4 pt-6">
                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${selectedOption === currentQuestion.correct_option ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400'}`}>
                      {selectedOption === currentQuestion.correct_option ? 'To\'g\'ri' : 'Noto\'g\'ri'}
                    </div>
                    <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500 text-xs">
                      <Target size={14} />
                      <span>43% to'g'ri javob bergan</span>
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full lg:hidden flex items-center justify-center gap-2 p-4 bg-[#1B4D3E]/10 text-[#1B4D3E] rounded-2xl border border-[#1B4D3E]/20 font-bold text-sm hover:bg-[#1B4D3E]/20 transition-all"
                >
                  <Brain size={18} />
                  AI Tushuntirishni ko'rish
                </button>
              </div>
            )}
          </div>
        ...
        <div className={`w-full lg:w-[55%] bg-[#0F172A] flex flex-col overflow-hidden border-l border-slate-800 ${(!isSubmitted || mobileView === 'question') && 'hidden lg:flex'}`}>
          <AnimatePresence mode="wait">
            {isSubmitted ? (
              ...
              >
                <div className="p-4 md:p-6 bg-[#1E293B]/50 border-b border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-[#1B4D3E] rounded-xl flex items-center justify-center shadow-lg shadow-[#1B4D3E]/20">
                      <Brain className="text-white" size={20} />
                    </div>
                    <h4 className="font-bold text-white text-base tracking-tight">AI Explanation</h4>
                  </div>
                  <button 
                ...
                <div className="flex-1 overflow-y-auto scrolling-touch p-4 md:p-8 space-y-6 md:space-y-8">
                  {aiLoading ? (
                    <div className="flex flex-col items-center justify-center h-full text-slate-500 gap-6">
                      <motion.div 
                        ...
                        className="w-20 h-20 bg-[#1B4D3E]/10 rounded-[32px] flex items-center justify-center border border-[#1B4D3E]/20"
                      >
                        <Brain size={32} className="text-[#1B4D3E]" />
                      </motion.div>
                      <div className="space-y-2 text-center">
                        <p className="text-xs font-bold uppercase tracking-[0.4em] text-slate-400">Tahlil qilinmoqda</p>
                        <div className="flex justify-center gap-1">
                          {[0, 1, 2].map(i => (
                            <motion.div 
                              ...
                              className="w-1.5 h-1.5 rounded-full bg-[#1B4D3E]"
                            />
                          ))}
                        </div>
                      </div>
                    ...
                    <>
                      ...
                      <section className="space-y-4">
                        <div className="flex items-center gap-3 text-blue-400">
                          ...
                          <h4 className="font-bold uppercase text-[11px] tracking-[0.2em]">Explanation</h4>
                        </div>
                        <div className="bg-[#1E293B]/50 p-5 md:p-8 rounded-[24px] md:rounded-[32px] border border-slate-800 shadow-xl backdrop-blur-sm">
                          <p className="text-slate-100 text-lg md:text-xl leading-relaxed whitespace-pre-wrap font-semibold">
                            {aiData?.explanation || currentQuestion.explanation_body}
                          </p>
                        </div>
                      </section>
                      ...
                      {(aiData?.keyPoints || currentQuestion.key_points) && (
                        <section className="space-y-4">
                          <div className="flex items-center gap-3 text-green-400">
                            ...
                            <h4 className="font-bold uppercase text-[11px] tracking-[0.2em]">Key Points</h4>
                          </div>
                          <div className="bg-[#1E293B]/50 p-5 md:p-8 rounded-[24px] md:rounded-[32px] border border-slate-800 shadow-xl backdrop-blur-sm space-y-4 md:space-y-6">
                            {(aiData?.keyPoints || currentQuestion.key_points || []).map((point, i) => (
                              <div key={i} className="flex gap-4 text-slate-100 text-base md:text-lg leading-relaxed font-semibold">
                                <div className="w-3 h-3 rounded-full bg-[#1B4D3E] mt-2 md:mt-2.5 shrink-0 shadow-[0_0_15px_rgba(27,77,62,0.6)]" />
                                {point}
                              </div>
                            ))}
                          </div>
                      ...
                      {aiData?.videoQuery && (
                        <section className="space-y-2">
                          <div className="flex items-center gap-2 text-red-600">
                            <Video size={16} />
                            <h4 className="font-bold uppercase text-[9px] tracking-widest">Video darslik</h4>
                          </div>
                          <a 
                          ...
                          >
                            <div className="flex items-center gap-2.5">
                              ...
                              <span className="text-[11px] font-bold text-slate-700">Mavzuga oid videoni ko'rish</span>
                            </div>
                            <ExternalLink size={12} className="text-slate-300 group-hover:text-red-400" />
                          </a>
                        </section>
                      )}
                        ...
                        <section className="space-y-4">
                          ...
                          <div className="bg-[#1E293B]/30 p-6 rounded-[32px] border border-slate-800/50 backdrop-blur-sm divide-y divide-slate-800/50">
                            {validSources.map((source, i) => {
                              ...
                              return (
                                <div key={i} className="py-5 first:pt-0 last:pb-0 flex items-start gap-4 group">
                                  <div className="w-10 h-10 rounded-full bg-slate-800/50 flex items-center justify-center text-sm font-mono text-slate-400 group-hover:bg-[#1B4D3E]/20 group-hover:text-[#1B4D3E] transition-colors shrink-0">
                                    {String(i + 1).padStart(2, '0')}
                                  </div>
                                  <div className="flex-1 space-y-3">
                                    <p className="text-slate-200 text-base font-semibold leading-relaxed group-hover:text-white transition-colors">
                                      {cleanSource}
                                    </p>
                                    {url ? (
                                      <a 
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-4">
                <div className="w-16 h-16 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex items-center justify-center">
                  <GraduationCap className="text-[#1B4D3E]" size={32} />
                </div>
                <div className="space-y-1">
                  <h4 className="font-bold text-slate-900 dark:text-white text-base">AI yordami kutilmoqda</h4>
                  <p className="text-slate-500 dark:text-slate-400 text-xs leading-relaxed max-w-[250px] mx-auto">
                    Javobingizni yuboring va AI sizga batafsil tushuntirish, vizual materiallar va manbalarni taqdim etadi.
                  </p>
                </div>
      ...
      {/* Footer Navigation */}
      <footer className="h-20 bg-[#1E293B] border-t border-slate-700 px-4 md:px-8 flex justify-between items-center shrink-0 safe-area-bottom">
        <button 
        ...
        <div className="flex-1 flex justify-center px-4">
          {!isSubmitted ? (
            <button
              ...
              className="w-full max-w-xs bg-[#1B4D3E] hover:bg-[#153a2f] text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-[#1B4D3E]/20 disabled:opacity-50 text-sm"
            >
              Javobni yuborish
            </button>
          ) : (
