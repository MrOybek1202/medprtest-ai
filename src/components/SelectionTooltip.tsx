...
export default function SelectionTooltip({
	...
	useEffect(() => {
		const handleMouseUp = (e: MouseEvent) => {
			...
			if (
				...
				if (
					tooltipRef.current &&
					tooltipRef.current.contains(selectionObj?.anchorNode || null)
				) {
					return
				}
				...
				if (containerId) {
					...
					if (
						container &&
						!container.contains(selectionObj?.anchorNode || null)
					) {
						setSelection(null)
						return
					}
	...
	const fetchDefinition = async () => {
		...
		setLoading(true)
		try {
			const data = await generateDefinition(
				selection.text,
				fullContext || selection.text,
			)
			...
			const errorMessage =
				error.message || "Ma'noni aniqlashda xatolik yuz berdi"
	...
	return (
		...
		>
			<motion.div
				initial={
					isMobile ? { opacity: 0, y: 50 } : { opacity: 0, scale: 0.9, y: -10 }
				}
					...
					<div className='p-6'>
						{loading ? (
							<div className='flex flex-col items-center py-6 gap-4 text-slate-400'>
								<Loader2 size={32} className='animate-spin text-[#102347]' />
							...
							<div className='space-y-4'>
								...
								{definition?.relatedTerms &&
									definition.relatedTerms.length > 0 && (
										<div className='flex flex-wrap gap-2 pt-2'>
											<span className='text-[10px] font-bold text-slate-400 uppercase tracking-wider w-full mb-1'>
												Bog'liq terminlar:
											</span>
											{definition.relatedTerms.map((term, i) => (
												<span
													key={i}
													className='text-[10px] font-medium text-blue-600 bg-blue-50 px-2 py-1 rounded-md'
												>
													{term}
												</span>
											))}
								...
								<div className='pt-2 flex gap-3'>
									<button
										...
										disabled={isSaved}
										className={`flex-1 py-3 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all ${isSaved ? 'bg-[#e6f0ff] text-[#2c5ff2] border border-[#b3d1ff]' : 'bg-slate-900 text-white hover:bg-slate-800 shadow-lg shadow-slate-200'}`}
									>
										{isSaved ? (
											<>
												<Check size={14} />
