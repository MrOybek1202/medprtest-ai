...
type UserRow = {
	...
	role: string
	created_at: string
	stats: { overall_accuracy: number; study_streak: number }[]
}
...
const dayKey = (isoDate: string | Date) => {
...
const buildHeatmapSeries = (sessions: SessionRow[]) => {
	...
	const points: { key: string; count: number }[] = []
	...
	for (let i = HEATMAP_DAYS - 1; i >= 0; i--) {
		...
		const key = dayKey(d)
		points.push({ key, count: countsByDay[key] || 0 })
	}
...
const heatColor = (count: number) => {
	...
	if (count <= 1) return 'bg-[#d9e5ff] dark:bg-[#1a2a52]'
	if (count <= 4) return 'bg-[#9bb9ff] dark:bg-[#23499e]'
	if (count <= 9) return 'bg-[#5f8cff] dark:bg-[#2c5ff2]'
	if (count <= 14) return 'bg-[#3e73f6] dark:bg-[#4f7ef7]'
	return 'bg-[#2c5ff2] dark:bg-[#74a0ff]'
}

export default function AdminPanel() {
	const [activeTab, setActiveTab] = useState<
		'questions' | 'users' | 'analytics' | 'topics'
	>('questions')
	const [questions, setQuestions] = useState<Question[]>([])
	const [users, setUsers] = useState<UserRow[]>([])
	const [userHeatmap, setUserHeatmap] = useState<Record<string, { key: string; count: number }[]>>({})
	...
	const fetchData = async () => {
		...
		try {
			...
			const { data: statsData, error: statsError } = await supabase
				.from('stats')
				.select('user_id, overall_accuracy, study_streak')
			...
			const statsMap = new Map<
				string,
				{ overall_accuracy: number; study_streak: number }
			>()
			for (const stat of statsData || []) {
				statsMap.set(stat.user_id, {
					overall_accuracy: stat.overall_accuracy || 0,
					study_streak: stat.study_streak || 0,
				})
			}

			const sessionsByUser: Record<string, SessionRow[]> = {}
			for (const session of sessionsData || []) {
				const list = sessionsByUser[session.user_id] || []
				list.push(session as SessionRow)
				sessionsByUser[session.user_id] = list
			}

			const mergedUsers: UserRow[] = (uData || []).map(user => {
				const userStats = statsMap.get(user.id) || {
					overall_accuracy: 0,
					study_streak: 0,
				}
				return {
					...(user as Omit<UserRow, 'stats'>),
					stats: [userStats],
				}
			})

			const heatmapMap: Record<string, { key: string; count: number }[]> = {}
			for (const user of mergedUsers) {
				heatmapMap[user.id] = buildHeatmapSeries(sessionsByUser[user.id] || [])
			}

			setUsers(mergedUsers)
			setUserHeatmap(heatmapMap)

			// Extract unique topics
			if (qData) {
				const uniqueTopics = Array.from(
					new Set(qData.map(q => q.topic)),
				).filter(Boolean)
	...
	const handleAddQuestion = async (e: React.FormEvent) => {
		e.preventDefault()
		try {
			const questionToSave = editingQuestion
				? { ...editingQuestion }
				: { ...newQuestion }
			...
			if (typeof keyPointsArray === 'string') {
				keyPointsArray = (keyPointsArray as string)
					.split(',')
					.map((s: string) => s.trim())
					.filter(Boolean)
			}
			...
			fetchData()
			toast.success(
				editingQuestion ? 'Savol yangilandi' : "Yangi savol qo'shildi",
			)
		} catch (error: any) {
			console.error('Error saving question:', error)
			toast.error(
				'Xatolik yuz berdi: ' + (error.message || 'Saqlashda xatolik'),
			)
		}
	}
	...
	const handleUpdateRole = async (userId: string, newRole: string) => {
		try {
			...
			toast.error(
				'Rolni yangilashda xatolik: ' + (error.message || "Noma'lum xatolik"),
			)
		}
	}
	...
	const accuracyData = [
		{
			range: '0-20%',
			count: users.filter(u => (u.stats?.[0]?.overall_accuracy || 0) <= 20)
				.length,
		},
		{
			range: '21-40%',
			count: users.filter(
				u =>
					(u.stats?.[0]?.overall_accuracy || 0) > 20 &&
					(u.stats?.[0]?.overall_accuracy || 0) <= 40,
			).length,
		},
		{
			range: '41-60%',
			count: users.filter(
				u =>
					(u.stats?.[0]?.overall_accuracy || 0) > 40 &&
					(u.stats?.[0]?.overall_accuracy || 0) <= 60,
			).length,
		},
		{
			range: '61-80%',
			count: users.filter(
				u =>
					(u.stats?.[0]?.overall_accuracy || 0) > 60 &&
					(u.stats?.[0]?.overall_accuracy || 0) <= 80,
			).length,
		},
		{
			range: '81-100%',
			count: users.filter(u => (u.stats?.[0]?.overall_accuracy || 0) > 80)
				.length,
		},
	]
	...
	const handleDeleteTopic = async (topic: string) => {
		try {
			...
			fetchData()
			toast.success(
				`"${topic}" mavzusi va unga tegishli barcha savollar o'chirildi`,
			)
			setDeleteConfirm(null)
		} catch (error: any) {
								...
								>
									...
									<div className='col-span-2 md:col-span-1 space-y-2'>
										...
										<input
											...
											value={
												(editingQuestion
													? editingQuestion.difficulty_level
													: newQuestion.difficulty_level) || 1
											}
											onChange={e =>
												editingQuestion
													? setEditingQuestion({
									...
									<div className='col-span-2 space-y-2'>
										...
										<textarea
											...
											value={
												(editingQuestion
													? editingQuestion.explanation_body
													: newQuestion.explanation_body) || ''
											}
											onChange={e =>
												editingQuestion
													? setEditingQuestion({
									...
									<div className='col-span-2 space-y-2'>
										<label className='text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest'>
											Asosiy nuqtalar (Key Points - vergul bilan ajrating)
										</label>
										<input
											className='w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-slate-200 dark:focus:ring-slate-700 outline-none text-slate-900 dark:text-white'
											value={
												editingQuestion
													? Array.isArray(editingQuestion.key_points)
														? editingQuestion.key_points.join(', ')
														: editingQuestion.key_points || ''
													: Array.isArray(newQuestion.key_points)
														? newQuestion.key_points?.join(', ')
														: newQuestion.key_points || ''
											}
											onChange={e =>
												editingQuestion
													? setEditingQuestion({
															...editingQuestion,
															key_points: e.target.value as any,
														})
													: setNewQuestion({
															...newQuestion,
															key_points: e.target.value as any,
									...
									<div className='col-span-2 flex justify-end gap-3 pt-4'>
										...
										<button
											type='submit'
											className='bg-slate-900 dark:bg-slate-800 text-white px-8 py-3 rounded-xl font-bold hover:bg-slate-800 dark:hover:bg-slate-700 transition-all'
										>
											Saqlash
										</button>
									</div>
											...
											.map(q => (
												...
												>
													<td className='px-6 py-4'>
														<span className='px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-full text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest'>
															{q.topic}
														</span>
													</td>
													<td className='px-6 py-4'>
														<p className='text-sm text-slate-900 dark:text-white font-medium line-clamp-2 max-w-md'>
															{q.stem}
														</p>
													</td>
													<td className='px-6 py-4'>
														<span className='w-6 h-6 bg-[#e6f0ff] dark:bg-[#2c5ff2]/20 text-[#2c5ff2] dark:text-[#2c5ff2] rounded-full flex items-center justify-center text-xs font-bold'>
															{q.correct_option}
														</span>
													</td>
						...
						<div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3'>
							...
							<div className='bg-white dark:bg-slate-900 p-10 rounded-[40px] border border-slate-200 dark:border-slate-800 shadow-sm'>
								...
								<p className='text-4xl font-bold text-slate-900 dark:text-white'>
									{Math.round(
										users.reduce(
											(acc, u) => acc + (u.stats?.[0]?.overall_accuracy || 0),
											0,
										) / (users.length || 1),
									)}
									%
							...
							<div className='bg-white dark:bg-slate-900 p-10 rounded-[40px] border border-slate-200 dark:border-slate-800 shadow-sm sm:col-span-2 md:col-span-1'>
								...
								<p className='text-4xl font-bold text-slate-900 dark:text-white'>
									{users.filter(u => u.stats?.[0]?.study_streak > 0).length}
								</p>
							</div>
						</div>
									...
									<tbody className='divide-y divide-slate-100 dark:divide-slate-800'>
										{filteredUsers.map(u => (
											...
											>
												...
												<td className='px-6 py-4'>
													<span
														className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${u.role === 'admin' ? 'bg-purple-100 dark:bg-purple-500/20 text-purple-700 dark:text-purple-400' : 'bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-400'}`}
													>
													...
													<div className='flex items-center gap-2'>
														<div className='w-16 h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden'>
															<div
																...
																style={{
																	width: `${u.stats?.[0]?.overall_accuracy || 0}%`,
																}}
															/>
														</div>
														<span className='text-xs font-bold text-slate-900 dark:text-white'>
															{u.stats?.[0]?.overall_accuracy || 0}%
														</span>
													</div>
												</td>
												<td className='px-6 py-4'>
													<div className='space-y-2'>
														...
														>
															{(userHeatmap[u.id] || []).map(point => (
																<div
																	key={point.key}
																	title={`${point.key}: ${point.count} test`}
																	className={`h-2.5 w-2.5 rounded-sm ${heatColor(point.count)}`}
																/>
															))}
														</div>
														<p className='text-[10px] text-slate-500 dark:text-slate-400'>
															Oxirgi {HEATMAP_DAYS} kun
														</p>
													</div>
												</td>
												<td className='px-6 py-4 text-right'>
					...
					<div className='space-y-4'>
						...
						<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2'>
							{topics.map(topic => (
								<div
									key={topic}
									className='bg-white dark:bg-slate-900 p-10 rounded-[40px] border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between group'
								>
									<div className='flex items-center gap-3'>
										...
										<div>
											...
											<p className='text-xs text-slate-500 dark:text-slate-400'>
												{questions.filter(q => q.topic === topic).length} savol
											</p>
										</div>
									</div>
									<button
										onClick={() =>
											setDeleteConfirm({ id: topic, type: 'topic' })
										}
