...
import { Search, Trash2, Book, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8 dark:bg-[#0F172A]">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white">Shaxsiy lug'at</h2>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Siz saqlagan barcha tibbiy terminlar va ularning ta'riflari.</p>
        </div>
      ...
      {filteredItems.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-[40px] border-2 border-dashed border-slate-200 dark:border-slate-800 p-20 text-center space-y-6">
          ...
          <div className="space-y-2">
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">Terminlar topilmadi</h3>
            <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto">
              Siz hali hech qanday termin saqlamagansiz yoki qidiruvga mos keladigan natija yo'q.
            </p>
          </div>
        </div>
      ) : (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white group-hover:text-[#1B4D3E] transition-colors">{item.term}</h3>
                  </div>
                  <button 
                ...
                <div className="flex justify-between items-center pt-2">
                  ...
                  <button className="flex items-center gap-1 text-[#1B4D3E] text-xs font-bold hover:underline">
